import{r as d,g as we,D as Ge,C as ve,s as qe,a as Ze,u as ut,l as pt,j as e,R as ht,b as ft,G as Ie,Y as Ae,c as Xe,d as mt}from"./index-ChbXbtze.js";import{TemplateRenderer as gt}from"./index-D8F_kZK6.js";import{u as _e}from"./githubSecrets-BW9mrW6G.js";const Qe=()=>new Promise((p,P)=>{if(window.JSZip){p(window.JSZip);return}const N=document.createElement("script");N.src="https://cdnjs.cloudflare.com/ajax/libs/jszip/3.10.1/jszip.min.js",N.onload=()=>p(window.JSZip),N.onerror=()=>P(new Error("Failed to load JSZip CDN")),document.head.appendChild(N)}),bt=async(p,P)=>{try{const N=await Qe(),x=new N;let g=await(await fetch("/")).text();g=g.replace("</head>",`<script>
    ...
    <\/script></head>`),g=g.replaceAll('src="/assets/','src="./assets/'),g=g.replaceAll('href="/assets/','href="./assets/'),g=g.replaceAll('href="/vendor/','href="./vendor/'),x.file("index.html",g),x.file("config.json",JSON.stringify(P,null,2));const B=Array.from(document.querySelectorAll('script[type="module"]')),j=Array.from(document.querySelectorAll('link[rel="stylesheet"]'));for(const C of j){const m=C.getAttribute("href");if(m&&m.startsWith("/assets/")){const O=await fetch(m);if(O.ok){const v=await O.text();x.file(m.substring(1),v)}}}const S=B.map(C=>C.getAttribute("src")).filter(C=>!!C&&C.startsWith("/assets/")),z=new Set;for(let C=0;C<S.length;C++){const m=S[C];if(z.has(m))continue;const O=await fetch(m);if(!O.ok)continue;const v=await O.text();x.file(m.substring(1),v),z.add(m);const b=[...v.matchAll(/from"\.\/([a-zA-Z0-9_.-]+\.js)"|import\("\.\/([a-zA-Z0-9_.-]+\.js)"\)/g)].map(o=>o[1]||o[2]);for(const o of b){const R=`/assets/${o}`;!z.has(R)&&!S.includes(R)&&S.push(R)}}const y=[...g.matchAll(/href="(?:\.\/)?(vendor\/[^"]+\.css)"/g)].map(C=>C[1]),Z=[...new Set(y)],f=new Set;for(const C of Z){if(f.has(C))continue;const m=await fetch(`/${C}`);if(!m.ok)continue;const O=await m.text();x.file(C,O),f.add(C);const v=[...O.matchAll(/url\(([^)]+)\)/g)].map(b=>b[1].replace(/['"]/g,""));for(const b of v){const o=new URL(b,`https://x/${C}`).pathname.slice(1);if(f.has(o))continue;const R=await fetch(`/${o}`);if(!R.ok)continue;const I=new Uint8Array(await R.arrayBuffer());x.file(o,I),f.add(o)}}const L=await x.generateAsync({type:"blob"}),E=document.createElement("a");E.href=URL.createObjectURL(L),E.download=`linkhubpage-${p}-profile.zip`,document.body.appendChild(E),E.click(),document.body.removeChild(E)}catch(N){console.error("ZIP packaging failed:",N),alert("Failed to generate ZIP export package.")}},xt=async(p,P)=>{try{const N=await Qe(),x=new N;x.file("wrangler.toml",`name = "linkhubpage-profile-\${username || 'custom'}"
main = "src/worker.ts"
...
`;x.file("src/worker.ts",W);let j=await(await fetch("/")).text();j=j.replace("</head>",`<script>
    ...
    <\/script></head>`),j=j.replaceAll('src="/assets/','src="./assets/'),j=j.replaceAll('href="/assets/','href="./assets/'),j=j.replaceAll('href="/vendor/','href="./vendor/'),x.file("public/index.html",j);const z={...P};delete z.resendApiKey,x.file("public/config.json",JSON.stringify(z,null,2));const y=Array.from(document.querySelectorAll('script[type="module"]')),Z=Array.from(document.querySelectorAll('link[rel="stylesheet"]'));for(const b of Z){const o=b.getAttribute("href");if(o&&o.startsWith("/assets/")){const R=await fetch(o);if(R.ok){const I=await R.text();x.file("public/"+o.substring(1),I)}}}const f=y.map(b=>b.getAttribute("src")).filter(b=>!!b&&b.startsWith("/assets/")),L=new Set;for(let b=0;b<f.length;b++){const o=f[b];if(L.has(o))continue;const R=await fetch(o);if(!R.ok)continue;const I=await R.text();x.file("public/"+o.substring(1),I),L.add(o);const $=[...I.matchAll(/from"\.\/([a-zA-Z0-9_.-]+\.js)"|import\("\.\/([a-zA-Z0-9_.-]+\.js)"\)/g)].map(J=>J[1]||J[2]);for(const J of $){const l=`/assets/${J}`;!L.has(l)&&!f.includes(l)&&f.push(l)}}const E=[...j.matchAll(/href="(?:\.\/)?(vendor\/[^"]+\.css)"/g)].map(b=>b[1]),C=[...new Set(E)],m=new Set;for(const b of C){if(m.has(b))continue;const o=await fetch(`/${b}`);if(!o.ok)continue;const R=await o.text();x.file("public/"+b,R),m.add(b);const I=[...R.matchAll(/url\(([^)]+)\)/g)].map($=>$[1].replace(/['"]/g,""));for(const $ of I){const J=new URL($,`https://x/${b}`).pathname.slice(1);if(m.has(J))continue;const l=await fetch(`/${J}`);if(!l.ok)continue;const u=new Uint8Array(await l.arrayBuffer());x.file("public/"+J,u),m.add(J)}}const O=await x.generateAsync({type:"blob"}),v=document.createElement("a");v.href=URL.createObjectURL(O),v.download=`linkhubpage-${p}-cloudflare-worker.zip`,document.body.appendChild(v),v.click(),document.body.removeChild(v)}catch(N){console.error("Worker ZIP packaging failed:",N),alert("Failed to generate Cloudflare Worker export package.")}},Le=p=>p.trim().toLowerCase().replace(/^https?:\/\//,"").replace(/\/.*$/,"").replace(/\.$/,"");function yt({username:p,profileConfig:P,persistBuilderDraft:N,getAuthToken:x,setDeployStatus:_,setStatusMsg:g}){const[W,B]=d.useState(!1),[j,S]=d.useState(""),[z,y]=d.useState("idle"),[Z,f]=d.useState(""),[L,E]=d.useState("idle"),[C,m]=d.useState(""),[O,v]=d.useState(""),[b,o]=d.useState(()=>`linkhubpage-profile-${p||"custom"}`),[R,I]=d.useState(""),[$,J]=d.useState(""),l=d.useRef(!1),u=d.useRef(!1),k=d.useRef(!1);d.useEffect(()=>{o(`linkhubpage-profile-${p||"custom"}`)},[p]);const re=async()=>{const a=Le(j);if(!a){y("invalid"),f("Please enter a valid domain first.");return}y("checking"),f("Checking whether the domain is using Cloudflare nameservers..."),E("idle"),m(""),I("");try{const r=await fetch("/api/cloudflare/domain-check",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({domain:a})}),s=await r.json();if(!r.ok){y("invalid"),f(s.error||"Domain check failed.");return}s.isCloudflare?(y("valid"),f(`Domain ${s.domain} is on Cloudflare. Opening Cloudflare login...`),$.trim()||J(`${s.domain}/*`)):(y("invalid"),f(`Domain ${s.domain} is not currently on Cloudflare nameservers.`))}catch(r){y("invalid"),f(r instanceof Error?r.message:"Unable to verify domain.")}},q=async()=>{const a=Le(j);if(z!=="valid"){E("error"),f("Please verify your domain on Cloudflare before login.");return}const r=O.trim();if(!r){E("error"),f("Please connect via the Cloudflare popup first.");return}E("connecting"),f("Connecting to Cloudflare and loading your account + zone details...");try{const s=await fetch("/api/cloudflare/connect",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({domain:a,apiToken:r})}),i=await s.json();if(!s.ok){E("error"),f(i.error||"Cloudflare login/connect failed.");return}m(i.accountId||""),I(i.zoneId||""),J(i.routePattern||`${a}/*`),E("connected"),f(`Connected to Cloudflare successfully. Zone: ${i.zoneName||a}`)}catch(s){E("error"),f(s instanceof Error?s.message:"Cloudflare login/connect failed.")}},xe=async()=>{var s,i,F;const a=Le(j);if(!a){alert("Please provide the domain you want to deploy to.");return}if(z!=="valid"){alert("Please verify that the domain is on Cloudflare first.");return}if(L!=="connected"){alert("Please login/connect Cloudflare to auto-load account and zone details.");return}if(!C||!O||!b||!R){alert("Missing Cloudflare deployment details. Please reconnect Cloudflare and try again.");return}const r=($||`${a}/*`).trim();_("loading"),g("Compiling your profile assets into a single-file Worker...");try{await N();const w={};(s=P.resendApiKey)!=null&&s.trim()&&(w.BUILDER_RESEND_API_KEY=P.resendApiKey.trim()),(i=P.googleClientId)!=null&&i.trim()&&(w.BUILDER_GOOGLE_CLIENT_ID=P.googleClientId.trim()),(F=P.googleAnalyticsId)!=null&&F.trim()&&(w.BUILDER_GOOGLE_ANALYTICS_ID=P.googleAnalyticsId.trim()),w.CLOUDFLARE_API_TOKEN=O.trim(),w.CLOUDFLARE_ACCOUNT_ID=C.trim(),w.CLOUDFLARE_SCRIPT_NAME=b.trim();const K={...P};delete K.resendApiKey,delete K.googleClientId,delete K.googleAnalyticsId;const ie=await fetch("/");if(!ie.ok)throw new Error("Failed to fetch HTML template.");let X=await ie.text();X=X.replace("</head>",`<script>
